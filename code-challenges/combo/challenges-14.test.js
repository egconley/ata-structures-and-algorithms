...
const toTitleCase = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    let stringSplit = arr[i].split('');
    stringSplit[0] = stringSplit[0].toUpperCase();
    let stringSmush = stringSplit.join('');
    // console.log(stringSmush);
    newArray.push(stringSmush);
  }
  return newArray;
};
...
let biggerThanLuke = (arr) => {
  let bigger = [];
  for (let i = 0; i < arr.length; i++) {
    if (parseInt(arr[i].mass) > 77) {
      bigger.push(arr[i].name);
    }
  }
  let biggerJoined = bigger.join(' - ');
  return biggerJoined;
};
...
const detectTicTacToeWin = (board) => {
  ...
  let diagFor = [row1[2], row2[1], row3[0]];
  let diagBack = [row1[0], row2[1], row3[2]];
  let win = [];
  let boardTest = [row1, row2, row3, col1, col2, col3, diagFor, diagBack];
  function pushToWin(arr) {
  ...
  boardTest.forEach(item => pushToWin(item));
